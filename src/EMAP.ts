...
import { fontDraw, fontLoaded } from './Draw3DText';

import { makeTextSprite, interpolate } from './tools';
...
export class EMAP {
  constructor(DOM, _options) {
    ...
    this.DOM = DOM;
    this.Planet = options.Planet ?? Earth;
  ...
  render() {
    ...
    this.objs.forEach((obj) => {
      obj.water.render(); /* obj.water.material.uniforms.time.value += 1.0 / 60.0; */
    });
  ...
  update() {
    ...
    this.camera.near = dist2 * 0.1;
    this.camera.far = dist2 * 1000;
  ...
  initHelpers() {
    ...
    const axisHelper = new THREE.AxisHelper(
      this.Planet.radius * this.rate * 1.2
    );
  ...
  _line(path, color = 0x666666) {
    ...
    path.forEach((p) => geometry.vertices.push(p));
  ...
  lookFar() {
    ...
    const PATH = interpolate(
      [g.fromV(this.fakeCamera.position), g.fromV(moveTo)],
      this.rate
    );
    ...
    this.__cameraUpPath = new THREE.SplineCurve3([
      this.__cameraUp,
      this.__north,
    ]);
    this.__lookAtPathT = 0;
